<template>
  <div class="container-fluid round-md">
    <!-- Prima sezione -->
    <div class="flex flex-col-reverse md:flex-row items-center md:min-h-[750px] h-full mt-4 p-6 bg-primaryDarker ">
      <div class="md:w-1/2">
        <img class="w-full h-full md:w-2/3 pt-6" src="/assets/images/mockup.png" alt="" />
      </div>
      <div class="md:w-1/2">
        <div class="text-3xl md:text-5xl font-semibold pb-6 text-white">
          AICS Lucca App!
          <div class="border-2 border-primaryLighter w-14"></div>
        </div>
        <div class="text-pretty text-white text-lg leading-8">
          Scopri la nuova App di AICS Lucca e rimani aggiornato in tempo reale su
          campionati, goal, classifiche e prossime partite. Utilizza il QR Code e
          scarica Gratuitamente la nostra nuova App per avere Sempre in tasca le
          ultime novità sul tuo universo sportivo!
        </div>
      </div>
    </div>
  </div>
</template>
